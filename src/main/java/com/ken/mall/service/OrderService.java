...
public class OrderService {
    ...
    public Order createOrder(String sku, int quantity) throws Exception {
        // Check for unpaid orders
        List<Order> unpaidOrders = orderRepository.findByStatus("pending");
        if (!unpaidOrders.isEmpty()) {
            throw new Exception("You have unpaid orders. Please complete payment before creating a new order.");
        }
        ...
        if (currentPrice != product.getPrice()) {
            throw new Exception("Price has changed. Please confirm the new price before placing the order.");
        }
